	...
	function checkPwr() { // passwordRepeat
		...
		if (pw.value.length > 5) { // pw > 5
			pw.setCustomValidity("N"); // false : "" 이면 false, "~"는 true
			pwr.setCustomValidity("N");
			var pNum=false;var pEng=false; // hasNum, hasEng
			...
			} else if (pwr.value.length == 1) { // pwr length 0
				out.innerHTML = "";
